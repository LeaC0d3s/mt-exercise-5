...
for corpus in train dev test; do
	subword-nmt apply-bpe -c $base/shared_models/4000_$src$trg.bpe --vocabulary $base/shared_models/vocab_4000.$src --vocabulary-threshold $bpe_vocab_threshold < $data/tok_prep2/$corpus.de-en.$src > $data/tok_prep2/$corpus.de-en.4000.bpe.$src
	subword-nmt apply-bpe -c $base/shared_models/4000_$src$trg.bpe --vocabulary $base/shared_models/vocab_4000.$trg --vocabulary-threshold $bpe_vocab_threshold < $data/tok_prep2/$corpus.de-en.$trg > $data/tok_prep2/$corpus.de-en.4000.bpe.$trg
done

# build joeynmt vocab
python $tools/joeynmt/scripts/build_vocab.py $data/tok_prep2/train.de-en.4000.bpe.$src $data/tok_prep2/train.de-en.4000.bpe.$trg --output_path $base/shared_models/vocab_4000.txt

# file sizes
for corpus in train dev test; do
	echo "corpus: "$corpus
	wc -l $data/tok_prep2/$corpus.de-en.4000.bpe.$src $data/tok_prep2/$corpus.de-en.4000.bpe.$trg
done
